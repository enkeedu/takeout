...
function placeholderImage(label: string, accent = "#0f766e") {
  ...
  <text x="450" y="300" text-anchor="middle" fill="${accent}" font-size="28" font-family="Cormorant Garamond, Georgia, serif" font-weight="600" letter-spacing="4">${safeLabel}</text>
  <text x="450" y="340" text-anchor="middle" fill="#6b7280" font-size="14" font-family="Work Sans, Arial, sans-serif" letter-spacing="3">IMAGE PLACEHOLDER</text>
</svg>`;
  const encoded = encodeURIComponent(svg);
  return `url("data:image/svg+xml;utf8,${encoded}")`;
}

export function TemplateNightMarket({
  restaurant,
  menu,
  reviews,
  gallery,
      ...
      <div className="relative left-1/2 right-1/2 -mx-[50vw] w-screen bg-[#f6f2ea]">
        ...
        <div className="relative mx-auto max-w-[1760px] px-6 py-10 space-y-10">
          <section className="full-bleed section-angled relative overflow-hidden rounded-[32px] border border-[var(--template-accent)] bg-[#f8f5ef] px-6 py-12 md:px-10">
            ...
            <Parallax
              strength={0.06}
              className="absolute -left-10 top-6 h-40 w-40 rounded-full bg-emerald-200/50 blur-3xl"
            />
            <Parallax
              strength={0.08}
              className="absolute -right-16 bottom-0 h-56 w-56 rounded-full bg-amber-200/40 blur-3xl"
            />
